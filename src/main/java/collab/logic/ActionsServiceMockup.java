...
public class ActionsServiceMockup implements ActionsService {
	...
	public Object invoke(ActionBoundary newAction) {
		...
		if (newAction.getElement() == null || newAction.getElement().getElementId().getDomain().isEmpty() || newAction.getElement().getElementId().getId().isEmpty())
			throw new RuntimeException("Element Id is nulls!");
		
		this.isValid(newAction.getInvokedBy().getUserId().getEmail());
		...
		User invokedBy = new User(new UserId(newAction.getInvokedBy().getUserId().getDomain(),newAction.getInvokedBy().getUserId().getEmail()));
		newAction.setInvokedBy(invokedBy);

		Element element = new Element(new ElementId(newAction.getElement().getElementId().getDomain(),newAction.getElement().getElementId().getId()));
